...
#[get("/user/<id>")]
pub fn get_user<'a>(id: u32, repository: &'a State<LegacyRepository>) -> Option<Json<User>> {
...
#[get("/user/all")]
pub fn get_all_users<'a>(repository: &'a State<LegacyRepository>) -> Json<Vec<User>> {
    Json(repository.get_all_users())
}

#[post("/user")]
pub fn add_user<'a>(session: Session, user: User, repository: &'a State<LegacyRepository>) -> MessageResponder<u32> {
